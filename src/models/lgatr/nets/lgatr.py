...
def encode_tokens(type_token, global_token, token_size, isgatr, batchsize, device):
    ...
    type_token = (
        type_token
        .expand(batchsize, *type_token.shape[1:])
        .float()
    )
    ...
    global_token = (
        global_token
        .expand(batchsize, *global_token.shape[1:])
        .float()
    )
...
class LGATr(Model):
    ...
    def forward(self, x):
        return self.amplitude_wrapper.forward(
            x,
            type_token = torch.tensor([self.type_token for _ in range(x.shape[0])]).to(torch.int64).to(x.device),
