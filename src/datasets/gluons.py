...
class gg_ng:
    ...
    def init_dataset(self):
        ...
        file_path = os.path.join(
            data_path, type, self.cfg[process]
        ) if not self.cfg.use_large_file else os.path.join(
            data_path, type, "large", self.cfg[process]
        )
        if not os.path.exists(file_path):
            ...
            try:
                momenta = np.load(file_path)[:n_events] if n_events is not None else np.load(file_path)
        ...
        if self.parameterisation.naive.use:
            events = [
                momenta[:, i : i + 4] for i in range(0, 4 * self.n_particles, 4)
            ]
            events.append(
    ...
    def apply_preprocessing(self, reverse=False, eps=1e-10):
        ...
        if not reverse:
            ...
            if pp_cfg.standardize:
                ...
                events_ppd[:, :-1] = (events_ppd[:, :-1] - self.mean) / (
                    self.std + eps
                )
            ...
            self.events = self.split_data(
                self.events, self.cfg.trn_tst_val
            )
            self.events_ppd = self.split_data(
                events_ppd, self.cfg.trn_tst_val
            )
            self.logger.info(
        ...
        if self.parameterisation.naive.use:
            for i in range(self.n_particles):
                ...
                self.observables.append(
                    Observable(
                        compute=lambda p, i=i: return_obs(p[..., :], p[..., 4 * i]),
                        tex_label=f"E_{{{type}}}" if 2<=i<=3 else f"E_{{{type}_{i+1 - 2}}}" if i > 3 else f"E_{{{type}_{i+1}}}",
                        unit="GeV",
                        bins=lambda obs: get_hardcoded_bins(
                            n_bins=n_bins + 1, lower=0, upper=1000
                        ),
                ...
                self.observables.append(
                    Observable(
                        compute=lambda p, i=i: return_obs(p[..., :], p[..., 4 * i + 1]),
                        tex_label=f"p_{{x, {type}}}" if 2<=i<=3 else f"p_{{x, {type}_{i+1 - 2}}}" if i > 3 else f"p_{{x, {type}_{i+1}}}",
                        unit="GeV",
                        bins=lambda obs: get_hardcoded_bins(
                            n_bins=n_bins + 1, lower=-500, upper=500
                        ),
                ...
                self.observables.append(
                    Observable(
                        compute=lambda p, i=i: return_obs(p[..., :], p[..., 4 * i + 2]),
                        tex_label=f"p_{{y, {type}}}" if 2<=i<=3 else f"p_{{y, {type}_{i+1 - 2}}}" if i > 3 else f"p_{{y, {type}_{i+1}}}",
                        unit="GeV",
                        bins=lambda obs: get_hardcoded_bins(
                            n_bins=n_bins + 1, lower=-500, upper=500
                        ),
                ...
                self.observables.append(
                    Observable(
                        compute=lambda p, i=i: return_obs(p[..., :], p[..., 4 * i + 3]),
                        tex_label=f"p_{{z, {type}}}" if 2<=i<=3 else f"p_{{z, {type}_{i+1 - 2}}}" if i > 3 else f"p_{{z, {type}_{i+1}}}",
                        unit="GeV",
                        bins=lambda obs: get_hardcoded_bins(
                            n_bins=n_bins + 1, lower=-500, upper=500
                        ),
...
def Gaussianize(x: torch.Tensor, pt_channels: list, phi_channels: list, eta_channels: list, mass_channels: list, reverse = False, eps: float = 1e-10) -> torch.Tensor:
    for ch in pt_channels:
        x[:, ch] = gaussianize_pt(
            x[:, ch], eps
        )
    for ch in phi_channels:
        x[:, ch] = gaussianize_phi(
            x[:, ch], eps
        )
    for ch in eta_channels:
        x[:, ch] = gaussianize_eta(
            x[:, ch], eps
        )
    for ch in mass_channels:
        x[:, ch] = gaussianize_mass(
            x[:, ch], eps
        )
    return x
def gaussianize_pt(x: torch.Tensor, epsilon: float, reverse: bool = False) -> torch.Tensor:
    if not reverse:
        return torch.log(x)
    else:
        return torch.exp(x.clip(max=10))

def gaussianize_phi(x: torch.Tensor, epsilon: float, reverse: bool = False) -> torch.Tensor:
    if not reverse:
        return phys.stable_arctanh(
            x / torch.pi, epsilon
        )
    else:
        return torch.tanh(x) * torch.pi

def gaussianize_eta(x: torch.Tensor, epsilon: float, reverse: bool = False) -> torch.Tensor:
    if not reverse:
        return x
    else:
        return x

def gaussianize_mass(x: torch.Tensor, epsilon: float, reverse: bool = False) -> torch.Tensor:
    if not reverse:
        return torch.log(x)
    else:
        return torch.exp(x.clip(max=10))
